...
function App() {
  ...
  const Button = ({ number }) => {
    return (
      <button
        ...
        className="bg-gray-700 h-10 w-10 rounded-full pt-1 text-gray-200 focus:bg-gray-400 btn-number trainsition-all duration-150"
      >
        {number}
      </button>
    );
  ...
  return (
    <>
      ...
      {isSubmitted && (
        <ThankYou itemNumber={itemNumber} setIsSubmitted={setIsSubmitted} />
      )}
    </>
  );
}

const ThankYou = ({ itemNumber, setIsSubmitted }) => {
  return (
    <div className="wrapper">
      ...
      <div className="flex items-center justify-center">
        <p className="p-rating bg-gray-700 rounded-full text-center text-sm px-4 pt-1">
          You selected {itemNumber} out of 5
        </p>
      </div>
